...
const SignupComponent = () => {
  ...
  const { values, handleBlur, handleChange, handleSubmit, touched, errors, resetForm } = useFormik({
    ...
    onSubmit: async () => {
      try {
        await axios
          .post('https://6a268ee12354b187.mokky.dev/register', values)
          .then((response) => {
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.content}>
        <p className="title">Создать аккаунт</p>
        <form onSubmit={handleSubmit}>
          <input
            ...
            className="input"
          />
          {errors.email && touched.email && <span className="error__message">{errors.email}</span>}
          <input
            type="text"
